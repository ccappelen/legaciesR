...
```{r plot panel grid, eval = TRUE, echo = FALSE, message = FALSE}
# Plot grid for each period
fig_panel_grid <- ggplot() +
  geom_sf(data = afr) +
  tidyterra::geom_spatraster(data = share_largest_count_panel, alpha = 0.7) +
  scale_fill_viridis_c(option = "plasma", direction = -1, na.value = NA) +
  labs(fill = "Share of polygons for the state with the largest \nnumber of polygons in total",
  ...
  facet_wrap(~ lyr)
ggsave(filename = "man/figures/README-plot panel grid-1.png",
       plot = fig_panel_grid)
```


```{r, eval = TRUE, echo = FALSE, message = FALSE}
knitr::plot_crop("man/figures/README-plot panel grid-1.png", quiet = TRUE)
knitr::include_graphics("man/figures/README-plot panel grid-1.png")
```
